...
def classify_log_iris():
    ...
    class_conditional_log_densities = []
    for hypothesized_class in [0, 1, 2]:  # Iterate over each hypothesized class
        ...
        class_conditional_log_densities.append(vrow(class_conditional_density))
    ...
    class_conditional_log_densities = np.vstack(class_conditional_log_densities)
    joint_log_densities = class_conditional_log_densities + prior_probs  # Add log prior to class-conditional log densities
    ...
    log_marginal = vrow(scipy.special.logsumexp(joint_log_densities, 0))

    # log_posterior_probabilities is the log-joint probability minus the log-marginal probability
    log_posterior_probabilities = joint_log_densities - log_marginal
    ...
    print(f"Accuracy: {num_correct_predictions / total_predictions}, Correct Predictions: {num_correct_predictions}, Total Predictions: {total_predictions}")
    return class_conditional_log_densities, posterior_probabilities
